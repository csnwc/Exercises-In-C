...
memory would be redundant. Here we investigate one of the many ways of
creating an alternative, where we trade-off speed for compactness: 
the Compact Sparse Array (CSA) which aims to
be memory efficient for sparsely occupied arrays.
...
\verb^uinit64_t^) showing which of the cells of the arrays are in use.
...
\end{itemize}
The CSA structure consists of a \verb^realloc()^'d array of blocks,
