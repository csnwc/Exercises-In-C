...
# count files in the zip (list files, process output to count only actual files)
l=`unzip -l nm.zip |& egrep -v "^-" | tail -n+3 | head -n-1 | wc -l`
if (( $l == 2 )); then
...
wget https://raw.githubusercontent.com/csnwc/Exercises-In-C/refs/heads/main/Code/Week8/NumMatch/Makefile
wget https://raw.githubusercontent.com/csnwc/Exercises-In-C/refs/heads/main/Code/Week8/NumMatch/driver.c
wget https://raw.githubusercontent.com/csnwc/Exercises-In-C/refs/heads/main/Code/Week8/NumMatch/nm.h
if (( $ext == 1 )); then
   wget https://raw.githubusercontent.com/csnwc/Exercises-In-C/refs/heads/main/Code/Week8/NumMatch/ext_driver.c
   wget https://raw.githubusercontent.com/csnwc/Exercises-In-C/refs/heads/main/Code/Week8/NumMatch/ext_nm.h
fi

# first try to make with gcc
make nm
if [[ ! -e nm ]]; then
   echo "ERROR: Failed to make nm with gcc?"
   exit
fi
make nm_d
if [[ ! -e nm_d ]]; then
   echo "ERROR: Failed to make nm_d with gcc?"
   exit
fi
...
if (( $ext == 1 )); then
   make extension
   if [[ ! -e extension ]]; then
      echo "ERROR: Failed to make extension with gcc?"
      exit
   fi
...
make nm
if [[ ! -e nm ]]; then
   echo "ERROR: Failed to make nm with clang?"
   exit
fi
make nm_d
if [[ ! -e nm_d ]]; then
   echo "ERROR: Failed to make nm_d with clang?"
   exit
fi
...
if (( $ext == 1 )); then
   make extension
   if [[ ! -e extension ]]; then
      echo "ERROR: Failed to make extension with clang?"
      exit
   fi
...
if (( $e == 0 )); then
   ...
   echo "ERROR: Error running nm?"
   exit
fi
if [[ -s output.txt ]]; then
   echo "ERROR: nm doesn't run silently?"
   exit
fi
./nm_d > output.txt 2>&1
e=$?
if (( $e == 0 )); then
   ...
   echo "ERROR: Error running nm_d?"
   exit
fi
if [[ -s output.txt ]]; then
   echo "ERROR: nm_d doesn't run silently?"
   exit
fi
